...
整体主题由 `data-theme` 和语义 CSS tokens 驱动，共提供晴空蓝、薄荷白、淡紫雾三套浅色主题，以及深海蓝、夜幕紫、墨绿青三套夜间主题，默认使用晴空蓝。主题目录、启动校验、即时应用与持久化统一由主题模块维护；启动时按本地有效值、数据库设置、默认主题的顺序恢复。主题只改变语义色，不改变布局或交互含义；正文与控件文字至少达到 WCAG 2.2 AA。

账号不使用人工标识色，头像是主要视觉识别方式；头像缺失时使用统一中性首字母占位。旧颜色字段只保留数据库兼容，不进入界面。
...
按钮、输入框、标签、列表行和导航统一使用 6–10px 圆角。账号详情使用右侧抽屉，在同一表面完成查看和编辑。每个交互控件具备 hover、focus-visible、active、disabled、loading 和 error 状态；窄窗口下侧栏折叠，不缩小正文到不可读尺寸。
