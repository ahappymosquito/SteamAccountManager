...
export const useUi: () => UiState = create<UiState>((set)=>({page:"accounts",query:"",favoriteOnly:false,platform:"",accountSort:"custom",selectedTags:[],notice:null,setPage:(page)=>set({page}),setQuery:(query)=>set({query}),setFavoriteOnly:(favoriteOnly)=>set({favoriteOnly}),setPlatform:(platform)=>set({platform,accountSort:platform==="5e"?"score_asc":"custom"}),setAccountSort:(accountSort)=>set({accountSort}),setSelectedTags:(selectedTags)=>set({selectedTags}),select:(selectedId)=>set({selectedId}),notify:(notice)=>set({notice})}));
