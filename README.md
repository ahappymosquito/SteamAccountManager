...
- 外出资料可用 TeamSpeak Unique ID 登录云存档并一键写入 CFG；网吧也可继续导入 U 盘资料包
...
软件备份为明文 JSON。恢复前先在应用数据目录静默保存当前资料，再按用户勾选的分类事务覆盖。账号类数据只按 SteamID64 恢复到本机当前可用账号。网吧请到「外出资料」用自己的 TeamSpeak Unique ID 拉取云存档，或导入 U 盘资料包：即使本机没有 Steam 凭证也能复制登录名、平台账号和 CFG，但不能切号。Unique ID 相当于存档钥匙。
