...
3. **读取身份、比赛列表和段位**
   ...
   - 应用按列表顺序选择最新一条 `level_info.level_name` 非空的有效天梯记录，跳过无段位的 1v1 等模式。“最近比赛后 ELO”取该记录根级或 `level_info` 中的 `origin_elo + change_elo`，段位与分数始终来自同一场比赛。因此它表示最近一场有效天梯比赛后的记录，不等同于 5E 官方页面此刻的实时数值。
