...
- 直接编辑多个 CS2 CFG 文件，按当前指令库刷新行尾注释，并复制准星命令；切号前按账号复制、校验和更新 `+exec` 启动参数
...
- 外出资料用自己起的短名字和口令登录云存档，打开后复制 Steam/5E/完美账号密码并写入 CFG；打开结果不写入本机库。网吧也可继续导入 U 盘资料包
...
软件备份为明文 JSON。恢复前先在应用数据目录静默保存当前资料，再按用户勾选的分类事务覆盖。账号类数据只按 SteamID64 恢复到本机当前可用账号。网吧请到「外出资料」用自己起的短名字和口令打开云存档，或导入 U 盘资料包：即使本机没有 Steam 凭证也能复制登录名、平台账号和 CFG，但不能切号。云存档打开后不写入本机库。口令是存档钥匙，不要告诉别人。
