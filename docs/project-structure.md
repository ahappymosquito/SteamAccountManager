...
- `pages/TravelPage.tsx`：外出资料卡，TeamSpeak Unique ID 云存档与 U 盘身份包。
...
| `ts3_identity.rs` | 读取本机 TeamSpeak 身份与 Unique ID |
| `vault.rs` | 按 Unique ID 上传、拉取云存档并部署 CFG |
| `scripts/vault-server/` | CDN 上的 Python 存档服务与 nginx/systemd 配置 |
