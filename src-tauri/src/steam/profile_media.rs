...
fn parse_profile_media(html: &str) -> ProfileMedia {
    let Some(start) = html.find("playerAvatarAutoSizeInner") else {
        return ProfileMedia {
            avatar_url: None,
            frame_url: None,
        };
    ...
    let frame_url = section
        ...
        .and_then(|frame_start| image_attribute(section, frame_start, "src"));
...
pub fn sync(cache_root: &Path, steam_ids: &[String], force: bool) -> AppResult<usize> {
    ...
    for steam_id64 in steam_ids {
        ...
        if let Ok(html) = profile {
            let media = parse_profile_media(&html);
